...
class BinTree:
    ...
    def find_node_special(self,node):
        if node.left is not None:
            return self.find_node_special(node.left)
        elif node.right is not None:
            return self.find_node_special(node.right)
        else:
            return node
    ...
    def insert(self,value):
        if self.root is None:
            ...
            position = input("Enter position (L for left, R for right)").upper()
            parent_node = self.find_node_special(self.root)
